(function () {
  ...
  function authShell(inner) {
    return `<div class="auth-wrap">
      ...
      <div class="brand-mark"><img class="logo-img" src="assets/logo.jpg" alt="logo">
  ...
  function playBoot() {
    return new Promise((resolve) => {
      ...
      wrap.innerHTML = `<video class="welcome-vid" autoplay muted loop playsinline src="assets/welcome.mp4"></video>
        <div class="auth-dim"></div>
        <div class="boot-inner glass"><img class="logo-img lg" src="assets/logo.jpg" alt="logo">
  ...
  function shell(html) {
    ...
    return `<header class="topbar">
      <div class="brand" data-go="home"><img class="logo-img sm" src="assets/logo.jpg" alt="logo">
  ...
  function viewHome() {
    ...
    const html = `
      <section class="banner" id="bannerBox">
        ...
        <div class="caption only-title"><h2>TOOLS 〆 NAILONG</h2>
        </div>
      </section>
  ...
  function viewTool(id) {
    ...
    app.innerHTML = shell(`<div class="tool-page nexus-tool">
      <button class="btn-sm" data-go="home">← Kembali</button>
      <div class="tool-hero glass">
        <div class="sub">${escapeHtml(t.category)}</div>
        <h2>${escapeHtml(t.name)}</h2>
        <p class="sub">${escapeHtml(t.description)}</p>
        <div class="tool-hero-ico"><i class="${t.icon}"></i></div>
      </div>
      <div class="tool-head" style="margin-top:8px">
        <button class="fav ${favOn?"on":""}" data-fav="${t.id}">★</button>
        <button class="btn-sm" id="shareTool">Bagikan</button>
      </div>
      ${body}
    </div>`);
    bindShell();
    ...
    $("#run")?.addEventListener("click", async () => {
      ...
      try {
        if (["word-counter","character-counter","sentence-counter","paragraph-counter","line-counter","caption-counter","bio-counter","text-stats"].includes(k) || t.id.includes("counter")) {
          ...
          let ans = "";
          if (/caption|status|quote/i.test(t.id) && window.NTAI_Caption) ans = NTAI_Caption.answer(v);
          else if (/hitung|calc|math|diskon/i.test(t.id) && window.NTAI_Hitung) ans = NTAI_Hitung.answer(v);
          else if (/resep|cook|recipe/i.test(t.id) && window.NTAI_Resep) ans = NTAI_Resep.answer(v);
          else ans = (window.NTLocalAI && NTLocalAI.answer(v, t.id)) || "";
          setOut(ans);
  ...
  async function fillStatusBar() {
    ...
    host.innerHTML = `<div class="status-card">
      ...
      <div class="status-main">
        <img class="status-ava-photo" src="${escapeHtml(USER?.photoURL || "assets/akun.jpg")}" alt="" onerror="this.src='assets/akun.jpg'">
        <div>
